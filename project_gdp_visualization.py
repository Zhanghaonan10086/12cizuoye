...
def render_world_map(gdpinfo, plot_countries, year, map_file):  # 将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    gdp_world_map.add(year, d1)
    gdp_world_map.add("missing from world bank", s1)
    gdp_world_map.add("no data at this year", s2)
...
def test_render_world_map(year):  # 测试函数
    ...
    render_world_map(gdpinfo, pygal_countries, year, "isp_gdp_world_name_1998.svg")
